...
@RestController
@RequestMapping("/api/plannedexpense")
public class PlannedExpenseController {

    @Autowired
    public PlannedExpenseService service;
    ...
    {
        try {
            List<PlannedExpense> response = service.list();
    ...
    {
        try {
            PlannedExpense response = service.findById(id);
            if (response == null)
            {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable long id, @RequestBody PlannedExpense input)
    {
        try {
