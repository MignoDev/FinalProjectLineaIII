...
import com.example.HomeFinances.Services.TypeExpenseService;
...
@RestController
@RequestMapping("/api/typeexpense")
public class TypeExpenseController {

    @Autowired
    public TypeExpenseService service;
    ...
    {
        try {
            List<TypeExpense> response = service.list();
    ...
    {
        try {
            TypeExpense response = service.findById(id);
            if (response == null){
    ...
    @PostMapping("/")
    public ResponseEntity<?> create(@RequestBody TypeExpense input){
        try {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable long id, @RequestBody TypeExpense input)
    {
        try {
